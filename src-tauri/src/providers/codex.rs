...
use crate::model::{
    RawSession, ServerToolUse, SessionMessage, SessionMessageRole, TokenCounts,
};

use super::{
    collect_jsonl_incremental, normalize_cache_inclusive, truncate, CollectResult, FileParseOutcome,
    Provider, RawUsage, ScanProgress, ScanProgressDelta, TITLE_MAX, TRIM_LIMIT,
};
...
fn extract_codex_messages(
    ...
    let mk = |role: SessionMessageRole, model: Option<String>, name: Option<String>, content: String| {
        SessionMessage {
            uuid: uuid.clone(),
...
mod tests {
    ...
    fn codex_response_message(id: &str, role: &str, ts: &str, content: serde_json::Value) -> serde_json::Value {
        serde_json::json!({
            "timestamp": ts,
            "type": "response_item",
    ...
    fn codex_session_title_extracts_vscode_ide_request() {
        ...
        write_jsonl(
            ...
            &[
                codex_session_meta_cwd("s1", "/tmp/project"),
                codex_response_message("m_u1", "user", "2026-07-10T03:00:10Z", serde_json::json!(ide)),
            ],
        );
        let p = CodexProvider::with_dir(dir.path().to_path_buf());
        let result = p.parse(&p.discover().unwrap()).unwrap();
        assert_eq!(result.sessions[0].title_orig, "Fix the session title preview");
    }
    ...
    fn codex_session_title_skips_vscode_context_without_request() {
        ...
        write_jsonl(
            ...
            &[
                codex_session_meta_cwd("s1", "/tmp/project"),
                codex_response_message("m_u1", "user", "2026-07-10T03:00:10Z", serde_json::json!(ide)),
                codex_response_message("m_u2", "user", "2026-07-10T03:00:11Z", serde_json::json!("Fix the login bug")),
            ],
        );
        let p = CodexProvider::with_dir(dir.path().to_path_buf());
        let result = p.parse(&p.discover().unwrap()).unwrap();
    ...
    fn codex_transcript_keeps_text_and_tool_call_drops_output() {
        ...
        write_jsonl(
            ...
            &[
                ...
                codex_response_message("m_u1", "user", "2026-07-10T03:00:10Z", serde_json::json!("list files")),
                codex_function_call("call_1", "2026-07-10T03:00:11Z", "shell", r#"{"cmd":["ls"]}"#),
                serde_json::json!({
        ...
        let asst = result.messages.iter().find(|m| m.role == Assistant).unwrap();
    ...
    fn codex_messages_incremental_past_cursor_with_stable_uuid() {
        ...
        write_jsonl(
            ...
            &[
                codex_session_meta_cwd("s1", "/tmp"),
                codex_response_message("m_u1", "user", "2026-07-10T03:00:10Z", serde_json::json!("first")),
            ],
        );
        let p = CodexProvider::with_dir(dir.path().to_path_buf());
        ...
        std::thread::sleep(std::time::Duration::from_millis(20));
        {
            ...
            writeln!(
                f,
                "{}",
                codex_response_message("m_u2", "user", "2026-07-10T03:05:00Z", serde_json::json!("second"))
            )
            .unwrap();
        }
        let (r2, _) = p.collect_incremental(&progress).unwrap();
    ...
    fn codex_subagent_session_emits_no_session_and_no_messages() {
        ...
        write_jsonl(
            ...
            &[
                ...
                codex_turn_context("gpt-5.6-sol"),
                codex_response_message("m_u1", "user", "2026-07-10T03:00:10Z", serde_json::json!("Inspect the project")),
                codex_token_count(100, 50, 10),
            ],
        );
        let p = CodexProvider::with_dir(dir.path().to_path_buf());
    ...
    fn codex_session_id_falls_back_to_filename_uuid() {
        ...
        write_jsonl(
            ...
            &[
                meta,
                codex_response_message("m_u1", "user", "2026-03-06T21:50:13Z", serde_json::json!("hello")),
            ],
        );
        let p = CodexProvider::with_dir(dir.path().to_path_buf());
        let result = p.parse(&p.discover().unwrap()).unwrap();
    ...
    fn codex_title_candidate_filters_noise_and_extracts_ide_request() {
        ...
        let ide_block = "# Context from my IDE setup:\n\n## My request for Codex:\nUse the real request";
        assert_eq!(
    ...
    fn codex_extract_message_text_handles_string_and_array() {
        assert_eq!(extract_codex_message_text(&serde_json::json!("plain")), "plain");
