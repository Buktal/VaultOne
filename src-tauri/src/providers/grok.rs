...
fn raw_session_from_summary(file: &Path, text: &str) -> RawSession {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(text) {
        ...
        if let Some(cwd) = v.get("info").and_then(|i| i.get("cwd")).and_then(|c| c.as_str()) {
        ...
        session.last_active_at = parse_grok_timestamp(
            v.get("last_active_at")
...
mod tests {
    ...
    fn grok_session_meta_from_summary_json() {
        ...
        assert_eq!(s.title_orig, "My Title", "generated_title beats session_summary");
    ...
    fn grok_session_last_active_falls_back_to_updated_at() {
        ...
        let result = p.parse(&p.discover().unwrap()).unwrap();
        assert_eq!(
            result.sessions[0].last_active_at,
            "2023-11-14T22:15:20.000Z",
    ...
    fn grok_timestamp_field_accepts_iso_epoch_seconds_and_millis() {
        ...
        let p = GrokProvider::with_dir(dir.path().to_path_buf());
        let result = p.parse(&p.discover().unwrap()).unwrap();
        let by_id: std::collections::HashMap<&str, &RawSession> = result
            .sessions
            .iter()
            .map(|s| (s.id.as_str(), s))
            .collect();
    ...
    fn grok_three_siblings_emit_one_session_messages_and_usages() {
        let dir = tempfile::tempdir().unwrap();
        let summary =
            r#"{"info":{"id":"s-full","cwd":"/proj"},"generated_title":"Full","created_at":1700000000,"last_active_at":1700000120}"#;
        let chat = [
            r#"{"type":"user","content":"hi","timestamp":1700000000}"#,
        ...
        let p = GrokProvider::with_dir(dir.path().to_path_buf());
        let result = p.parse(&p.discover().unwrap()).unwrap();
        assert_eq!(result.sessions.len(), 1, "exactly one session (no duplicate)");
    ...
    fn grok_incremental_chat_history_emits_only_appended_messages() {
        ...
        let session_dir =
            write_grok_full_session(dir.path(), "s-inc", Some(summary), &chat, &[]);
