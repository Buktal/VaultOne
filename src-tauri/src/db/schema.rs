...
/// (architecture.md: invariants in code). `local_group_id` is device-private
/// (never enters git); the rest sync via the `session-meta` grain.
...
/// `local_groups` — device-private group names (local track; ADR 0002). Never
/// enters git; CRUD is immediate (no network).
