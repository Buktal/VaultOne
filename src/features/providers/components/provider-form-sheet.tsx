...
// carries `${VAR}` placeholders (the Bedrock presets). On save the fields map
// onto the provider's settingsConfig snapshot via the derive.ts helpers
// (withBasicFields preserves every field the form doesn't own), then the
// upsert mutation runs and the sheet closes.
//
...
// field (they are a quick view of `env`), and editing a field merges it back
// into the JSON. The merge is guarded by `parseJsonObject` — while the JSON
// text does not parse to an object, field edits only update the field state
// and the in-progress JSON edit is never clobbered; once it parses again the
// fields re-derive. Save still goes through withBasicFields, so the field
// values win for the keys they own and everything else survives verbatim.
