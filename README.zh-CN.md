...
- **读取你工具已产生的日志** —— 直接解析 Claude Code 留在磁盘上的会话日志。无需代理、无需 API key、无需联网即可使用看板。
...
- **轻量速览模式** —— 缩成屏幕边缘的半图标，悬停即可瞥见今日用量，无需打开完整看板。
- **托盘常驻、后台采集** —— 增量扫描器在后台让看板保持新鲜，无需保留窗口。
- **Per-call 与 Per-turn 双视角** —— 下钻到每一次模型调用（成本、时长、`stop_reason` 语义），并汇总到整回合的成本与墙钟耗时。
...
- **用量趋势** —— 双 Y 轴的 token 对成本随时间变化曲线，支持切换图例。
...
- **当前 Claude Code，更多 provider 规划中** —— 采集器按可插拔的 provider 架构设计。
...
- **轻量速览模式** —— 贴边缩成半图标 + 悬停瞥见今日用量。
- **自定义标题栏、浅色 / 深色主题。**
- **默认私密** —— 除非你开启同步，用量数据始终留在你的机器上。
...
- **采集（Collect）** 读取本机源日志，解析后写入本地库（并产出同步产物）。
- **本地库（Local store）** 是看板读取的查询层。
- **同步（Sync，opt-in）** 在设备间经 GitHub 仓库 push / pull 产物。
...
yarn dist           # 构建发布版桌面二进制
```

| 命令 | 说明 |
| --- | --- |
| `yarn dev` | 以开发模式运行完整的 Tauri 桌面应用（Vite + Rust）。 |
| `yarn web:dev` | 仅运行 Web UI（Vite），用于纯前端迭代。 |
| `yarn check` | 全部静态检查——前端（Biome + tsc）与 Rust（fmt + clippy），与 CI 同构。 |
| `yarn web:fix` | 自动修复前端 lint 与格式（Biome）。 |
| `yarn web:build` | 类型检查并构建 Web 产物。 |
| `yarn test` | 运行全部测试（Rust 测试套件）。 |
| `yarn dist` | 构建发布版桌面二进制。 |
